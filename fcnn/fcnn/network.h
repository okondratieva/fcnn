...
float random()
{
	return (rand() / float(RAND_MAX)-0.5)*2*0.1;
}
...
public:
	FCNetwork(int size, int hiddenLayerSize) {
		...
		derivatives[0] = new float[sizes[0]]; //for output from first layer
		derivatives[1] = new float[sizes[1]]; //for output from softmax layer

		weights[0] = new float[(sizes[0]+1)*sizes[1]];
		weights[1] = new float[(sizes[1]+1)*sizes[2]];

		for (int i = 0; i < (sizes[0]+1) * sizes[1]; i++) {
			weights[0][i] = random();
		}

		for (int i = 0; i < (sizes[1]+1)* sizes[2]; i++) {
			weights[1][i] = random();
		}
	...
	void softmax() {
		...
		for (int i = 0; i < sizes[2]; i++) {
			for (int j = 0; j < sizes[1]; j++) {
				outs[2][i] += weights[1][i*(sizes[1] + 1) + j] * outs[1][j];
			}
			outs[2][i] += weights[1][i*(sizes[1] + 1) + sizes[1]];
			tmp = expf(outs[2][i] - max);
			sum += tmp;
			outs[2][i] = tmp;
		}
		sum = 1.f / sum;
	...
	void directStep() {
		...
		for (int i = 0; i < sizes[1]; i++) {
			for (int j = 0; j < sizes[0]; j++) {
				outs[1][i] += weights[0][i*(sizes[0]+1) + j] * outs[0][j];
			}
			outs[1][i] += weights[0][i*(sizes[0] + 1) + sizes[0]];
			outs[1][i] = tanh(outs[1][i]); //activation function
		}
	...
	void backPropagation(float learningRate, int label) {
		...
		for (int i = 0; i < sizes[2]; i++) {
			for (unsigned int j = 0; j < sizes[1]; j++) {
				weights[1][i*(sizes[1]+1) + j] -= derivatives[1][i] * outs[1][j] * learningRate;
			}
			weights[1][i*(sizes[1] + 1) + sizes[1]] -= derivatives[1][i] * learningRate;
		}
		...
		for (int i = 0; i < sizes[1]; i++)
		{
			...
			for (unsigned int j = 0; j < sizes[2]; j++)
				derivatives[0][i] += derivatives[1][j] * weights[1][j*(sizes[2]+1) + i];
		...
		for (int i = 0; i < sizes[1]; i++) {
			for (unsigned int j = 0; j < sizes[0]; j++) {
				weights[0][i*(sizes[0]+1) + j] -= derivatives[0][i] * outs[0][j] * learningRate;
			}
			weights[0][i*(sizes[0]+1) + sizes[0]] -= derivatives[0][i] * learningRate;
		}
	}
